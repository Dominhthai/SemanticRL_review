...
# Original author: lukun199@gmail.com
# Review implementation by Dominhthai
*/

#include "CiderD.h"

	
Cider::Cider(SVV_type cache, int n, int sigma): n(n), sigma(sigma) {
	update_df(cache);
}


float Cider::dict_query(std::string s){
	if(document_frequency.find(s)!=document_frequency.end()) return document_frequency[s];
	return 0.0;
}

void Cider::dict_update(std::string s){
	if(document_frequency.find(s)!=document_frequency.end()) document_frequency[s]+=1;
	else document_frequency[s]=1;
}
...
void Cider::update_df(SVV_type cache){
	for(SV_type ref: cache){
		crefs.push_back(cook_refs(ref));
	}
	compute_doc_freq();
    ref_len = log(float(crefs.size()));
    clear();
}
...
std::tuple<std::vector<std::unordered_map<std::string, float>>, std::vector<float>, float> Cider::counts2vec(std::unordered_map<std::string, int> cnts){
	...
	for(std::pair<std::string, int> p:cnts){
		...
		float df=log(std::max(float(1.0), dict_query(ngram)));
